...
public class GraphController {
    ...
    */
     ...
     * @param contaminant true if contaminant PPM is to be displayed
     *                    in the graph
     */
    public void setGraphOptions(int year, boolean virus, boolean contaminant) {
        ...
        List<WaterQualityReport> reports = Model.getInstance().
                getQualityReportsByYear(year);
        ...
        if (contaminant) {
            ...
            for (WaterQualityReport report : reports) {
                monthPPMs[report.getDateMonth() - 1] += report.
                        getContaminantPPM();
                count[report.getDateMonth() - 1]++;
            }
            for (int i = 0; i < 12; ++i) {
